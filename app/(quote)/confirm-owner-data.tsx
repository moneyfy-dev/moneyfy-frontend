...
export default function ConfirmOwnerDataScreen() {
  ...
  const selectedInsurerName =
    typeof selectedPlan?.insurer === 'string'
      ? selectedPlan.insurer
      : selectedPlan?.insurer?.name ?? '';
  const selectedDeductibleDesc =
    selectedPlan?.deductibleDesc?.trim() || `Deducible ${selectedPlan?.deductible ?? 0} UF`;
  ...
  const validateForm = () => {
    ...
    if (formData.ownerName && !validateName(sanitizeName(formData.ownerName))) {
    ...
    if (
      ...
      !validateName(sanitizeName(formData.ownerPaternalSur))
    ) {
    ...
    if (
      ...
      !validateName(sanitizeName(formData.ownerMaternalSur))
    ) {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedPlanId || selectedPlanId.length < 4) {
      setFormError(
        'El plan seleccionado no trae un identificador válido. Vuelve a seleccionar el plan desde los resultados.',
      );
      return;
    }
    ...
    try {
      await selectPlan({
        ...
        ownerName: sanitizeName(formData.ownerName),
        ownerPaternalSur: sanitizeName(formData.ownerPaternalSur),
        ownerMaternalSur: sanitizeName(formData.ownerMaternalSur),
      ...
      setFormError(
        hiddenErrors.length > 0
          ...
          : responseStatus === 424
            ? 'Esta cotización ya no está disponible para continuar. Vuelve a cotizar para generar una nueva sesión.'
            : responseMessage
        ...
        >
          <View style={styles.content}>
            ...
            <ThemedText variant="paragraph" textAlign="center" marginBottom={16}>
              Los datos del propietario se utilizan para comunicaciones importantes como la carta
              de cancelación y para ajustar la cobertura y condiciones del seguro según la
            ...
            <ThemedAutocomplete
              ...
              placeholder={formData.city ? 'Selecciona una comuna' : 'Selecciona primero una región'}
